...
import cn.keking.model.FileInfo;
...
public class FileUtils {
    ...
    public FileUtils(CacheService cacheService, FilePathConfig filePathConfig) {
    ...
    /**
     * 根据code从文件服务器中获取文件实际路径
     *
     * @param code
     * @return
     */
    public String getFileDownloadUrl(String code, String token) throws GetFileException {
        StringBuilder sb = new StringBuilder();
        if (code != null && !code.equals("")) {
            try {
                sb.append("http://").append(filePathConfig.getFileIP()).append(":").append(filePathConfig.getFilePort()).append(filePathConfig.getFileURL()).append(code);
                URL url = new URL(sb.toString());
                HttpURLConnection open = (HttpURLConnection) url.openConnection();
                open.setRequestProperty("Access-Token", token);
                int respCode = open.getResponseCode();
                if (respCode == 200) {
                    String json = IOUtils.toString(open.getInputStream(), StandardCharsets.UTF_8);
                    FileInfo fileInfo = JSON.parseObject(json, FileInfo.class);
                    if (fileInfo != null) {
                        StringBuilder stringBuilder = new StringBuilder();
                        String fileDomain = stringBuilder.append("http://").append(filePathConfig.getFileIP()).append(":").append(filePathConfig.getGoFastDFSPort()).toString();
                        return fileInfo.getDiskPath().replaceAll(".*:[\\d]+", fileDomain);
                    } else {
                ...
                e.printStackTrace();
                throw new GetFileException("获取文件信息异常：" + e.getMessage());
            }
        }
        throw new GetFileException("code不能为空");
    }
}
